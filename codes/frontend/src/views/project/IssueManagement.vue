...
import { getMyIssues, createIssue, updateIssueStatus } from '@/api/issue'
...
async function handleStatusChange(row: MyIssueItem, newStatus: number) {
  try {
    await updateIssueStatus(row.projectId, row.id, { status: newStatus })
  ...
  <div class="issue-management">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;flex-wrap:wrap;gap:8px">
      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
        ...
        <el-select v-model="filters.projectId" placeholder="所属项目" clearable filterable size="small" style="width:180px" @change="fetchData">
      ...
      <el-button type="primary" size="small" @click="dialogVisible = true">新建问题</el-button>
    </div>

    <el-table :data="list" border size="small" v-loading="loading" max-height="calc(100vh - 240px)">
      <el-table-column label="所属项目" width="180">
        <template #default="{ row }">
          <el-button link size="small" type="primary" @click="goToProject(row.projectId)">{{ row.projectCode }} - {{ row.projectName }}</el-button>
        </template>
      </el-table-column>
    ...
    <IssueDialog :visible="dialogVisible" :show-project-select="true"
